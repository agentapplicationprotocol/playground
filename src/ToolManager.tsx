...
export default function ToolManager({ clientTools, onClientToolsChange, serverTools, onServerToolsChange, clientToolsSupported }: Props) {
  ...
  function save() {
    if (!editing) return;
    try {
      const schema = typeof editing.spec.inputSchema === "string"
      ...
      if (editIndex !== null) {
        const next = [...clientTools]; next[editIndex] = tool; onClientToolsChange(next);
      } else {
        ...
        <div className="tool-bar">
          ...
          {serverTools.map((t, i) => (
            <span key={t.name} className={`tool-chip server-tool ${t.enabled ? "" : "disabled"}`}>
              <span onClick={() => toggleEnabled(i)} title="Toggle enabled">{t.name}</span>
              {t.enabled && (
                <span className={`trust-badge ${t.trust ? "trusted" : ""}`} onClick={() => toggleTrust(i)} title="Toggle trust">
      ...
      {editing ? (
        <div className="tool-editor">
          <div className="tool-editor-row">
            <label>Name<input value={editing.spec.name} onChange={(e) => setEditing((t) => ({ ...t!, spec: { ...t!.spec, name: e.target.value } }))} /></label>
            <label>Description<input value={editing.spec.description} onChange={(e) => setEditing((t) => ({ ...t!, spec: { ...t!.spec, description: e.target.value } }))} /></label>
          </div>
          <label>Input Schema (JSON)
            <textarea rows={4} value={typeof editing.spec.inputSchema === "string" ? editing.spec.inputSchema as unknown as string : JSON.stringify(editing.spec.inputSchema, null, 2)}
              onChange={(e) => setEditing((t) => ({ ...t!, spec: { ...t!.spec, inputSchema: e.target.value as unknown as ToolSpec["inputSchema"] } }))} />
            {schemaError && <span className="error">{schemaError}</span>}
          </label>
          <label>Function body <span className="hint">(receives <code>input</code>, must return a string)</span>
            <textarea rows={4} value={editing.code} onChange={(e) => setEditing((t) => ({ ...t!, code: e.target.value }))} />
          </label>
          <div className="tool-editor-actions">
            <button onClick={save} disabled={!editing.spec.name}>Save</button>
            <button className="disconnect" onClick={() => setEditing(null)}>Cancel</button>
            {editIndex !== null && <button className="disconnect" onClick={() => { removeClient(editIndex); setEditing(null); }}>Delete</button>}
          </div>
        </div>
      ) : (
